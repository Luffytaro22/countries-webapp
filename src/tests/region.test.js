...
it('should update the state on successful API call', async () => {
  ...
  expect(store.getState().regions.regions[region]).toEqual('27,657,145');
});

it('should dispatch the correct action on API call failure', async () => {
  const region = 'Non-existent Region';
  mockAxios.onGet('https://api.apilayer.com/geo/country/region/Non-existent%20Region').reply(404);

  try {
    await store.dispatch(getRegions(region));
  } catch (error) {
    expect(error.message).toEqual('Region not found');
  }
});
